...
def num_splits(s, d):
    ...
    """
    "*** YOUR CODE HERE ***"
    def sum_splits_iter(e, s, d):
        if sum(e) > sum(s):
            return 0.0
        if s == []:
            if sum(e) + d >= sum(s):
                return 0.5 if sum(e) == 0 else 1.0      # if sum(e) == 0, means there have repeat
            return 0.0                                  # combination of subset, eg. [1, 2], [3] and [3], [1, 2] 
        return sum_splits_iter(e+[s[0]], s[1:], d) + sum_splits_iter(e+[-s[0]], s[1:], d)
        ###    # set e select element s[0]           # set e does not select elemnt s[0]
    return int(sum_splits_iter([], s, d))
...
class Account:
    ...
    def __repr__(self):
        ...
        for x in self.transactions:
            if x[0] == 'deposit':
                deposit += 1
        return f"Accountholder: {self.holder}, Deposits: {deposit}, Withdraws: {len(self.transactions)-deposit}"
...
def reverse(lst):
    ...
    """
    "*** YOUR CODE HERE ***"
    accumulator = Link.empty
    while lst is not Link.empty:
        accumulator = Link(lst.first, accumulator)
        lst = lst.rest
    return accumulator
