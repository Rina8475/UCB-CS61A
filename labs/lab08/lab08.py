...
def subseqs(s):
    ...
    if not s:
        ...
        prev = subseqs(s[:-1])
        return subseqs(s[:-1]) + [x + [s[-1]] for x in prev]

def is_non_decrease(s):
    """Judge if a sequence is non decrease"""
    if len(s) < 2:
        return True
    return all([s[x] <= s[x+1] for x in range(len(s)-1)])
...
def non_decrease_subseqs(s):
    ...
    if not s:
        ...
        tem = subseqs(s[:-1])
        tem.extend([x + [s[-1]] for x in tem])
        return [x for x in tem if is_non_decrease(x)]
    # def subseq_helper(s, prev):
    #     if not s:
    #         return [[], [prev]]
    #     elif s[0] < prev:
    #         return subseq_helper(s[1:], prev) + subseq_helper(s[1:], s[0])
    #     else:
...
def num_trees(n):
    ...
    *   *        *       *
    ...
    note, x = [0, 1], 2
    while x <= n:
        note.append(sum([note[y] * note[x-y] for y in range(1, x)]))
        x += 1
    return note[n]
...
class Account:
    ...
    def __repr__(self):
        "*** YOUR CODE HERE ***"
        deposit_count, withdraw_count = 0, 0
        for trans in self.transactions:
            if trans[0] == 'deposit':
                deposit_count += 1
            else:
                withdraw_count += 1
        return f'Accountholder: {self.holder}, Deposits: {deposit_count}, Withdraws: {withdraw_count}'
...
def long_paths(t, n):
    ...
    "*** YOUR CODE HERE ***"
    if t.is_leaf():
        return [] if n > 0 else [[t.label]]
    seq = []
    for ch in t.branches:
        seq += insert_into_all(t.label, long_paths(ch, n-1))
    return seq
