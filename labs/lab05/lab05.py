...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(city, city_a) <= distance(city, city_b) \
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        return True
    return any(berry_finder(sub) for sub in branches(t))
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    # return [deck[x+y] for x in range(len(deck)//2) for y in [0, len(deck)//2]]
    return [deck[x//2] if x%2 == 0 else deck[(len(deck)+x)//2] for x in range(len(deck))]
...
def add_trees(t1, t2):
    ...
    list_t1 = branches(t1)
    list_t2 = branches(t2)
    min_len = min(len(list_t1), len(list_t2))
    return tree(label(t1)+label(t2), [add_trees(x, y) for x, y in zip(list_t1, list_t2)]
            + list_t1[min_len:] + list_t2[min_len:])
