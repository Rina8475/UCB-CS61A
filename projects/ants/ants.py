...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                ...
                place.ant = self
            elif place.ant.can_contain(self) and place.ant.contained_ant is None:   # 注意第二个条件
    ...
    def buff(self):
        ...
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        if hasattr(self, 'damage') and not hasattr(self, 'has_buffed'):
            self.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place, count = self.place, 0.0
        while (place is not beehive) and (count < self.min_range):
            place = place.entrance
            count += 1.0
        while (place is not beehive) and (count <= self.max_range):
            if place.bees != []:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        damage = amount if amount < self.health else amount + self.damage
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt defend the colony behind he"""
...
# The HungryAnt Class
class HungryAnt(Ant):
    """HungryAnt eat a bee in his place"""
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        """Return if this ant can contain the other ant"""
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        return not (other.is_container() and other.contained_ant is not None)
        # END Problem 8
    ...
    def buff(self):
        """ContainerAnt buff equals to the ant it contained buff"""
        if self.contained_ant is not None:      # 仅当包含 contained ant 时进行处理
            self.contained_ant.buff()
        super().buff()
...
class BodyguardAnt(ContainerAnt):
    ...
    def add_to(self, place):
        if place.ant is not None and type(self) == type(place.ant):     # 相同的两个 BodyguardAnt 不能放于同一格
            assert place.ant is None, 'Two ants in {0}'.format(place)
        super().add_to(place)
    # END Problem 8
...
class TankAnt(ContainerAnt):
    """TankAnt not only protect other Ants, but offend the bees in the 
    same place as well"""
    ...
    def action(self, gamestate):
        for bee in list(self.place.bees):       # 在对遍历时会对其进行修改的序列遍历时，应遍历其副本
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower is a kind of ThrowerAnt who can live in water"""
    name = 'Scuba'
    food_cost = 6
    is_watersafe = True
    implemented = True
...
# END Problem EC
    ...
    def __init__(self, health=1):
        # BEGIN Problem EC
        "*** YOUR CODE HERE ***"
        self.is_queen = not QueenAnt.has_queen
    ...
    def action(self, gamestate):
        ...
        while place is not None:
            if place.ant is not None:       # 仅当当前位置有 ant 时进行处理
                place.ant.buff()
            place = place.exit
        super().action(gamestate)
        # END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.is_queen and self.health <= 0:
            bees_win()
        # END Problem EC
...
class Bee(Insect):
        ...
        def constructor(f):
            def status(self, gamestate):
                if gamestate.time % 2 == 0:
                    f(gamestate)
            return status
        self.apply_status(constructor(self.action), self.action, length)
        # END Problem Optional 2
    ...
    def apply_status(self, status, previous_action, length):
        ...
        # BEGIN Problem Optional 2
        "*** YOUR CODE HERE ***"
        def instead_action(self, gamestate):
            nonlocal length
            status(self, gamestate)
            length -= 1
            if length == 0:
                self.action = previous_action
        self.action = MethodType(instead_action, self)

        # def instead_action(self, gamestate):
        #     status(self, gamestate)
        #     self.apply_status(status, previous_action, length-1)
        # self.action = MethodType(instead_action, self) if length > 0 else previous_action
