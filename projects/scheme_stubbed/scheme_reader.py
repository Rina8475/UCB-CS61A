...
def scheme_read(src):
    ...
    # BEGIN PROBLEM 1/2
    "*** YOUR CODE HERE ***"
    current = src.pop_first()
    if isinstance(current, int) or isinstance(current, float) or isinstance(current, bool):
        return current
    if current == 'nil':
        return nil
    if current not in DELIMITERS:   # symbols
        return current
    # delimiters
    if current == '(':
        return read_tail(src)       # dry
    if current in quotes:
        return Pair(quotes[current], Pair(scheme_read(src), nil))
    raise SyntaxError
    # END PROBLEM 1/2
